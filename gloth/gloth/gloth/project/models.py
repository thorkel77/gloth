...
class Choix_medicaments(db.Model):
    __tablename__="choix_medicaments"
    __bind__="choix_medicaments"
    id = db.Column(db.Integer(), primary_key=True)
    user_id = db.Column(db.Integer(), db.ForeignKey('user.id', ondelete='CASCADE'))
    icd_10 = db.Column(db.Integer(), db.ForeignKey('icd_10', ondelete='CASCADE'))
    cis = db.Column(db.Integer(), db.ForeignKey('cis', ondelete='CASCADE'))
...
class Treatment_class(db.Model):
